...
export default function OrdersList() {
  ...
  const{orders:{orders},error,loading} = useSelector(state => state?.orders)
                  ...
                  <tr key={order._id}>
                    ...
                    <td className="hidden px-3 py-4 text-sm text-gray-500 lg:table-cell">
                      {order.paymentStatus === "Not paid" ? (
                        <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-300 text-green-800">
                          Not paid
                        </span>
                      ) : (
